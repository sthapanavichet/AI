...
def findModelUnderstandingCheck() -> Dict[Any, bool]:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # clone of Expr to represent 'a' as a literal
    class Expression:
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # condition for unsatisfiable
    condition = premise & ~conclusion
    # if condition is satisfied when calling findModel, it means that the premise doesnt entail the conclusion
    if findModel(condition):
        return False
    return True
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # by disjoining the literals, if at least one of the literal is true, the whole statement equates to true.
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    pairs = list()
    result = list()
    # get all combinations of the literals in the list
    for i in range(0, len(literals)):
        for j in range(i+1, len(literals)):
            pairs.append((literals[i], literals[j]))

    # put all the pairs into a conjunction and then negates it because if any pair is true, it equates to false.
    # this will make the whole statement false
    for pair in pairs:
        # result.append(~(pair[0] & pair[1]))
        # & cannot be used because the final statement has to be in CNF, so we use de morgan's law
        result.append(~pair[0] | ~pair[1])
