...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # all possible position on the map for pacman and ghost to be at
    possiblePositions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    # maximum distance a ghost and a pacman can be
    maxDistance0 = max([manhattanDistance(pacPos, ghost0Pos) for pacPos in possiblePositions for ghost0Pos in
                        possiblePositions])
    maxDistance1 = max([manhattanDistance(pacPos, ghost1Pos) for pacPos in possiblePositions for ghost1Pos in
                        possiblePositions])
    variableDomainsDict = {
        PAC: possiblePositions,
        GHOST0: possiblePositions,
        GHOST1: possiblePositions,
        # domain for OBS0 and OBS1 is from 0 to maximum distance + maximum noise 
        OBS0: [obs0 for obs0 in range(MAX_NOISE + maxDistance0 + 1)],
        OBS1: [obs1 for obs1 in range(MAX_NOISE + maxDistance1 + 1)]
    }
